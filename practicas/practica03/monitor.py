...
def read_token():
    try:
        fd = open("/home/alumnos/alonsod/lagrs/practica03/token.txt")
    except IOError:
...
def send_messages(dirname, size, nfiles, force):
    ...
    if size > MAX_TAMANIO and nfiles > MAX_FICHEROS:
        flag = False
        msg_error_all = hostname + ": Warning: in dir "+ dirname + ":\nBad total size: " + \
